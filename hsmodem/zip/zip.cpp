...
static mz_int64 zip_file_move(MZ_FILE *m_pFile, const mz_uint64 to,
  ...
  if (fread(move_buf, 1, (size_t)length, m_pFile) != length) {
    MZ_FCLOSE(m_pFile);
    return ZIP_EFREAD;
  }
  ...
  if (fwrite(move_buf, 1, (size_t)length, m_pFile) != (size_t)length) {
    MZ_FCLOSE(m_pFile);
    return ZIP_EFWRITE;
  }
...
static mz_int64 zip_files_move(MZ_FILE *m_pFile, mz_uint64 writen_num,
  ...
  while ((mz_int64)length > 0) {
    ...
    n = (int)zip_file_move(m_pFile, writen_num, read_num, move_count, move_buf,page_size);
    if (n < 0) {
      moved_length = n;
      goto cleanup;
    }
...
static int zip_central_dir_move(mz_zip_internal_state *pState, int begin,
  ...
  if (l_size == 0) {
    memmove(pState->m_central_dir.m_p, next, (size_t)r_size);
    pState->m_central_dir.m_p = MZ_REALLOC(pState->m_central_dir.m_p, (size_t)r_size);
  ...
  if (l_size * r_size != 0) {
    memmove(deleted, next, (size_t)r_size);
    for (int i = end; i < entry_num; i++) {
      MZ_ZIP_ARRAY_ELEMENT(&pState->m_central_dir_offsets, mz_uint32, i) -= (mz_uint32)d_size;
    }
  }

  pState->m_central_dir.m_size = (size_t)(l_size + r_size);
  return 0;
}
...
ssize_t zip_stream_copy(struct zip_t *zip, void **buf, ssize_t *bufsize) {
  ...
  if (bufsize != NULL) {
    *bufsize = (ssize_t)zip->archive.m_archive_size;
  }
  *buf = calloc(sizeof(unsigned char), (size_t)zip->archive.m_archive_size);
  memcpy(*buf, zip->archive.m_pState->m_pMem, (size_t)zip->archive.m_archive_size);
